...
@Composable
fun SettingsPlaybackMaxAudioChannelsScreen() {
	...
	SettingsColumn {
		item {
			ListSection(
				...
				captionContent = { Text(stringResource(R.string.pref_max_audio_channels_description)) },
			)
		}

		items(MaxAudioChannels.entries) { entry ->
			ListButton(
				headingContent = { Text(stringResource(entry.nameRes)) },
				trailingContent = { RadioButton(checked = maxAudioChannels == entry) },
				onClick = {
					maxAudioChannels = entry
					router.back()
				}
			)
		}
